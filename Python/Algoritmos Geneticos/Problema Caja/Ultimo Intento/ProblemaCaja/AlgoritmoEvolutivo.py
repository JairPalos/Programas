...
class AlgoritmoEvolutivo:
    ...
    def showPob(self, showAptitude=False):#imprime la poblacion
        ...
        for i in range(self.size):#ciclo en el tamaño de la poblacion
            if showAptitude:#si es true
                ...
                print("H,W,L --> "+str(self.pob.poblacion[i].getValues()))
                print("volumen es: "+ str(np.prod(self.pob.poblacion[i].getValues())))
                #print(np.prod(self.pob.poblacion[i].getValues()))
            else:#si es false
    ...
    def elmejor(self):
        ...
        print("H,W,L --> "+str(self.pob.poblacion[idxMejor].getValues()))
        print("volumen es: "+ str(np.prod(self.pob.poblacion[idxMejor].getValues())))
        print("\n")
        
    def graf(self):
        poblacion = self.pob.poblacion
        aptitudes = [self.ff.evaluate(ind) for ind in poblacion]
        axes =[] #GUARDA VALORES PARA GRAFICAR
        mejores=[]#VALORES APTITUD MAYOR
        #ELIMINA VALORES REPETIDOS
        aptitudes = list(set(aptitudes))
        #ORDENA
        aptitudes.sort(reverse=True)
        #ELIMINA VALORES REPETIDOS DE LA POBLACION
        self.pob.poblacion=list(set(self.pob.poblacion))
        
        for i in range(5):
           #VALORES MAYOR APTITUD
           mejores=np.argmax(aptitudes)
           #obtiene valores de l,w,h de cada poblacion
           axes.append(self.pob.poblacion[mejores].getValues())
           #borra valores de la poblacion y aptitudes 
           self.pob.poblacion.pop(mejores)
           aptitudes.pop(mejores)
        #print(aptitudes)
            #axes.poblacion.pop(mejores)
        #    aptitudes.pop(mejores)
        for i in range(5):
            #valores de h,w,l
            axe=axes[i]
            vol=axe[0]*axe[1]*axe[2]
            ...
            # utilizamos el método bar3d para graficar la caja

            ax.bar3d(z, y, x, dx, dy,dz)
            plt.title("volumen {}".format(vol))
            print("-----------------------")
            print("Caja",i+1)
            print("h",axe[0])
            print("l",axe[2])
